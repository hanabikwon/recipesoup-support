...
4. **Puppeteer MCP 테스트** (Browser Tests)
5. **성능 및 접근성 테스트**
...
await puppeteer_navigate({ 
  ...
  launchOptions: { headless: false } // 테스트 과정 시각적 확인
});
...
// 3. 메인 테스트 실행
await puppeteer_evaluate({ 
  script: fs.readFileSync('./tests/puppeteer_food_analysis_test.js', 'utf8')
});
```
...
await puppeteer_evaluate({
  script: `
    // FAB 버튼 클릭
    await page.click('[data-testid="fab"], .fab, #fab');
    await page.waitForSelector('[data-testid="fab-menu"]', {timeout: 5000});
    
    // "사진으로 작성" 선택
    await page.click('[data-testid="photo-recipe-btn"]');
    await page.waitForSelector('[data-testid="recipe-create"]');
...
await puppeteer_evaluate({
  script: `
    ...
    await photoInput.uploadFile('./testimg1.jpg');
    ...
    // 분석 결과 대기 (최대 20초)
    await page.waitForSelector('[data-testid="analysis-result"]', {timeout: 20000});
...
### 4.3 전체 Puppeteer 테스트 실행
```bash
# Node.js 환경에서 직접 실행 (개발용)
cd tests
node puppeteer_food_analysis_test.js
...
# 6. Puppeteer MCP 테스트 (수동 실행 안내)
echo "6️⃣ Puppeteer MCP 테스트 준비 완료"
echo "   다음 명령으로 실행하세요:"
echo "   - 테스트 이미지를 tests/ 디렉토리에 복사"
echo "   - MCP 도구에서 puppeteer 테스트 스크립트 실행"
...
- [x] **Flutter Web Puppeteer 테스트 PASS**
- [x] **커버리지 목표 달성** (85% 이상)
...
4. **브라우저 호환성**: Puppeteer MCP로 실제 사용자 시나리오 테스트

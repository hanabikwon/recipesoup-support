...
- **AI/ML**:
  ...
  - 모델: [gpt-4o-mini via Vercel serverless proxy]
- **데이터 저장**: [로컬]
...
2. **[핵심 기능 2. AI 음식 분석 및 날짜 자동 기록]**: [선택적으로 음식 사진을 AI로 분석하여 레시피를 자동 추천받을 수 있습니다. 사진은 분석 후 즉시 삭제되며 저장되지 않습니다. 요리한 날짜를 자동으로 저장하거나 수동 선택할 수 있어 시간순 회고, 감정 흐름 파악에 용이합니다.]
...
4. **[핵심 기능 4. 레시피 리스트 및 상세 보기]**: [기록된 레시피들은 카드형 목록으로 정리되며, 각 레시피를 클릭하면 감정 메모, 재료, 조리법, 태그 등 전체 내용을 열람할 수 있습니다. 감정과 기억의 흐름을 시각적으로 돌아볼 수 있는 개인 아카이브 역할을 합니다.]

## Vercel 프록시 서버 설정 (보안 강화)
```yaml
# Vercel 서버리스 환경변수 (실제 운영 중)
Projects → recipesoup-proxy → Settings → Environment Variables:
  OPENAI_API_KEY: [서버리스 환경변수에서 안전 관리]
  API_MODEL: gpt-4o-mini
  PROXY_APP_TOKEN: [앱 전용 인증 토큰]

# 클라이언트 측 구성 (lib/config/api_config.dart)
baseUrl: https://recipesoup-proxy-*.vercel.app

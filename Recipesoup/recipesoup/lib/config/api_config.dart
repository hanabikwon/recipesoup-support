...
''';
  ...
  static Map<String, dynamic> createSingleIngredientRecipeRequest({
    ...
    return {
      ...
      'max_tokens': maxTokens ?? 800,
      'temperature': 0.7, // 다양성을 위해 창의적인 temperature
    };
  }
  ...
  static String createSingleIngredientRecipePrompt(List<String> ingredients) {
    ...
    return '''
다음 재료들로 만들 수 있는 요리 1개를 추천해주세요: $ingredientsText
...
3. **난이도와 조리 시간도 다양하게**:
   - 간단한 요리 (10-20분): 계란요리, 간단한 볶음 등
   - 보통 요리 (20-40분): 찌개, 볶음밥, 면 요리 등
   - 정성 요리 (40분 이상): 조림, 찜, 베이킹 등

4. 입력된 재료를 최대한 활용하되, 추가 재료는 일반적으로 구하기 쉬운 것들로 제한해주세요
5. 조리 단계는 명확하고 실용적으로 3-7단계로 작성해주세요
6. 각 재료는 구체적인 양과 단위를 포함해주세요
7. JSON 형식을 정확히 준수해주세요

**예시:**
- 같은 재료(돼지고기, 김치)로도 "김치찌개", "김치볶음", "김치전", "김치덮밥" 등 다양한 조리법 가능
- 닭고기로 "닭볶음탕", "치킨샐러드", "치킨스테이크", "닭가슴살구이" 등 스타일 다양화
''';
  }

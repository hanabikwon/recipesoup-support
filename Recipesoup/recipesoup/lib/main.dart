...
// 🚨 Force-close 감지 및 Hive 동기화 핸들러 (SceneDelegate와 연동)
void _setupForceCloseHandler() {
  const platform = MethodChannel('com.recipesoup.app/lifecycle');

  platform.setMethodCallHandler((call) async {
    // RELEASE 모드에서도 로그 출력 (CRITICAL!)
    print('📱 Flutter: Received method call from iOS: ${call.method}');

    if (call.method == 'forceFlushHiveBoxes') {
      // CRITICAL: SceneDelegate가 호출 (iOS 13+ 앱 종료 시)
      print('🚨 Flutter: Force-flushing all Hive boxes (SceneDelegate call)');

      try {
        final startTime = DateTime.now().millisecondsSinceEpoch;
        await _flushAllHiveBoxes();
        final duration = DateTime.now().millisecondsSinceEpoch - startTime;

        print('✅ Flutter: All Hive boxes flushed successfully (${duration}ms)');
        return 'success'; // ✅ iOS SceneDelegate에 성공 응답
      } catch (e) {
        print('❌ Flutter: Error flushing Hive boxes: $e');
        return 'error: $e'; // ❌ iOS SceneDelegate에 에러 응답
      }
    } else if (call.method == 'flushHiveBoxesOnBackground') {
      // BONUS: Background 진입 시에도 Hive 동기화
      print('📱 Flutter: Flushing Hive boxes on background');

      try {
        await _flushAllHiveBoxes();
        print('✅ Flutter: Background flush completed');
        return 'success';
      } catch (e) {
        print('❌ Flutter: Background flush error: $e');
        return 'error: $e';
      }
    }

    print('⚠️ Flutter: Unknown method call: ${call.method}');
    return null; // 알 수 없는 메서드
  });
}

// 모든 Hive Box 동기화
Future<void> _flushAllHiveBoxes() async {
  try {
    print('🔥 FLUSH START: Force-flushing all Hive boxes (${DateTime.now().millisecondsSinceEpoch})');

    // ✅ CRITICAL FIX: Box가 열려있는지 먼저 확인
    final List<Future<void>> flushFutures = [];

    if (Hive.isBoxOpen(AppConstants.recipeBoxName)) {
      // 🔥 TEST 17: Box 타입을 dynamic으로 변경
      final recipesBox = Hive.box<dynamic>(AppConstants.recipeBoxName);
      print('🔥 FLUSH: recipes box (${recipesBox.length} items)');
      flushFutures.add(recipesBox.flush());
    } else {
      print('⚠️ FLUSH: recipes box not open, skipping');
    }

    if (Hive.isBoxOpen(AppConstants.settingsBoxName)) {
      final settingsBox = Hive.box(AppConstants.settingsBoxName);
      print('🔥 FLUSH: settings box');
      flushFutures.add(settingsBox.flush());
    } else {
      print('⚠️ FLUSH: settings box not open, skipping');
    }

    if (Hive.isBoxOpen(AppConstants.statsBoxName)) {
      final statsBox = Hive.box(AppConstants.statsBoxName);
      print('🔥 FLUSH: stats box');
      flushFutures.add(statsBox.flush());
    } else {
      print('⚠️ FLUSH: stats box not open, skipping');
    }

    if (Hive.isBoxOpen(AppConstants.burrowMilestonesBoxName)) {
      // 🔥 TEST 17: Box 타입을 dynamic으로 변경
      final burrowMilestonesBox = Hive.box<dynamic>(AppConstants.burrowMilestonesBoxName);
      print('🔥 FLUSH: burrowMilestones box');
      flushFutures.add(burrowMilestonesBox.flush());
    } else {
      print('⚠️ FLUSH: burrowMilestones box not open, skipping');
    }

    if (Hive.isBoxOpen(AppConstants.burrowProgressBoxName)) {
      // 🔥 TEST 17: Box 타입을 dynamic으로 변경
      final burrowProgressBox = Hive.box<dynamic>(AppConstants.burrowProgressBoxName);
      print('🔥 FLUSH: burrowProgress box');
      flushFutures.add(burrowProgressBox.flush());
    } else {
      print('⚠️ FLUSH: burrowProgress box not open, skipping');
    }

    print('🔥 FLUSH: Starting parallel flush (${flushFutures.length} boxes)');

    // ✅ CRITICAL FIX: 이미 수집한 futures만 실행
    if (flushFutures.isNotEmpty) {
      await Future.wait(flushFutures);
      print('✅ FLUSH COMPLETE: ${flushFutures.length} boxes flushed (${DateTime.now().millisecondsSinceEpoch})');
    } else {
      print('⚠️ FLUSH: No boxes open to flush');
    }

    // ⚠️ COMPACT 제거: Force-close 시에는 flush만 실행 (속도 최적화)
    // compact()는 파일을 완전히 재작성하므로 시간이 오래 걸림 (2초 타임아웃 원인)
    // NativeHiveSync의 fsync()가 이미 디스크 동기화를 보장하므로 불필요
    print('⏭️ COMPACT SKIPPED: Only flush() executed for fast termination');
  } catch (e) {
    print('❌ FLUSH ERROR: $e');
    rethrow;
  }

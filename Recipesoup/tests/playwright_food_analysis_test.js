/**
 * Recipesoup 음식 사진 분석 테스트 (Playwright MCP)
 * 
 * 이 스크립트는 Playwright MCP를 사용하여 Recipesoup 앱의 
...
async function testAppAccessibility() {
  ...
  await addTest('앱 로딩 테스트', async () => {
    // Playwright MCP 방식으로 페이지 이동
    console.log('  🌐 페이지 이동 중...');
    
    // 스플래시 화면 확인
    const splashExists = await page.locator('.splash-screen, #splash').first().isVisible().catch(() => false);
    
    if (splashExists) {
      console.log('  ✅ 스플래시 화면 로딩됨');
      
      // 메인 화면 전환 대기
      await page.locator('.main-screen, #main-screen, [data-testid="main-screen"]').waitFor({
        timeout: 10000
      });
  ...
  await addTest('Bottom Navigation 확인', async () => {
    ...
    const bottomNav = await page.locator('.bottom-navigation, #bottom-navigation').first();
    ...
    const navTabs = await page.locator('.bottom-nav-item, .nav-tab, [data-testid*="nav-tab"]').count();
  ...
  await addTest('FAB 버튼 확인', async () => {
    ...
    const fab = await page.locator('.floating-action-button, #fab, [data-testid="fab"]').first();
    await fab.waitFor({ timeout: 5000 });
    
    const isVisible = await fab.isVisible();
    if (!isVisible) {
...
async function testImageAnalysis(imageKey, imageData) {
  ...
  await addTest(`${imageKey} 업로드 및 분석`, async () => {
    ...
    await page.locator('.floating-action-button, #fab, [data-testid="fab"]').click();
    
    // FAB 확장 메뉴에서 "사진으로 작성" 선택
    const photoRecipeBtn = page.locator('[data-testid="fab-photo-recipe"], .fab-photo-recipe, #fab-photo-recipe').first();
    
    try {
      await photoRecipeBtn.waitFor({ timeout: 5000 });
      await photoRecipeBtn.click();
    } catch {
      // 직접 레시피 작성 화면으로 이동
      await page.locator('.recipe-create, #recipe-create, [data-testid="recipe-create"]').waitFor();
    }
    
    console.log('  ✅ 레시피 작성 화면 진입');
    
    // 2. 이미지 업로드
    const photoInput = page.locator('input[type="file"], #photo-input, [data-testid="photo-input"]').first();
    await photoInput.waitFor({ timeout: 5000 });
    
    const isVisible = await photoInput.isVisible();
    if (!isVisible) {
      throw new Error('사진 업로드 input을 찾을 수 없습니다');
    }
    
    await photoInput.setInputFiles(path.resolve(imageData.path));
    console.log(`  ✅ 이미지 업로드: ${imageData.path}`);
    
    // 3. OpenAI 분석 시작 표시 대기
    try {
      const analysisLoader = page.locator('.analysis-loading, #analysis-loading, [data-testid="analysis-loading"]').first();
      await analysisLoader.waitFor({ timeout: TEST_CONFIG.imageUploadTimeout });
    ...
    // 4. 분석 결과 대기
    const analysisResult = page.locator('.analysis-result, #analysis-result, [data-testid="analysis-result"]').first();
    await analysisResult.waitFor({ timeout: TEST_CONFIG.apiAnalysisTimeout });
    
    const isResultVisible = await analysisResult.isVisible();
    if (!isResultVisible) {
...
async function validateAnalysisResult(imageData) {
  ...
  await addTest(`재료 추천 검증 (${imageData.description})`, async () => {
    const ingredientsSection = page.locator('.suggested-ingredients, #suggested-ingredients, [data-testid="suggested-ingredients"]').first();
    
    try {
      await ingredientsSection.waitFor({ timeout: 5000 });
  ...
  await addTest(`조리법 추천 검증 (${imageData.description})`, async () => {
    const instructionsSection = page.locator('.suggested-instructions, #suggested-instructions, [data-testid="suggested-instructions"]').first();
    
    try {
      await instructionsSection.waitFor({ timeout: 5000 });
...
// Playwright MCP에서 사용할 전역 변수들
let page;

/**
 * Playwright MCP에서 호출할 메인 함수
 * 
 * 사용법:
